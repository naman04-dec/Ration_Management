...
class govRationListView(ListView):
    model=govRation
...
def govRationForm1(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            ob=models.govRation(shop_name=form.cleaned_data['shop_name'],aadhar_num=form.cleaned_data['aadhar_num'],sugar_quant=form.cleaned_data['sugar_quant'],wheat_quant=form.cleaned_data['sugar_quant'],rice_quant=form.cleaned_data['sugar_quant'],date=datet)
            ob.save()
